...
function formatAmount(amount: number, currency: string): string {
  if (!amount) return ''
  if (currency === 'BTC') return amount.toLocaleString('es-AR', { minimumFractionDigits: 5, maximumFractionDigits: 8 })
...
interface Props {
  ...
  loading: boolean
  error: string
  /** Cuántos skeletons mostrar mientras carga (default 4) */
  skeletonCount?: number
  /** Mensaje cuando no hay items */
  emptyMessage?: string
}

export default function MovimientosList({ items, loading, error, skeletonCount = 4, emptyMessage }: Props) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      {loading && Array.from({ length: skeletonCount }).map((_, i) => <SkeletonRow key={i} />)}
      ...
      {!loading && !error && items.length === 0 && (
        <div style={{ padding: '64px 0', textAlign: 'center', fontFamily: 'Inter, sans-serif', fontSize: 14, color: C.mutedDark }}>
          {emptyMessage ?? 'Todavía no realizaste ninguna operación.'}
        </div>
      )}
    </div>
  )
